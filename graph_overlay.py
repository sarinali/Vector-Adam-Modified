...
class SphereSimulation:
    def __init__(self, radius: float, projected: List[float], unprojected: List[float]) -> None:
        self.radius = radius
...
for i in range(1, 11):
    ...
    radius = float(lines[cur_line].strip())
...
for i in range(len(simulations)):
    plt.plot(x, simulations[i].projected, label=f"Radius {i+1} (Gradient: Projected, Momentum: Rotated)", alpha=0.2, color="blue")
    plt.plot(x, simulations[i].unprojected, label=f"Radius {i+1} (Gradient: Projected, Momentum: Projected)", alpha=0.2, color="green")
...
plt.title("Loss over different sphere radiuses")
# plt.legend()
