...
## this file compares the VectorAdam Modified algorithm when it projects the momentum vs when it does not. The purpose is to find the difference in gradient vectors
...
for i in range(steps):
    ...
    loss2.backward()
    vadam_unnormalized.step_modified(v_unprojected, project_momentum=False)
    ...
    v_unprojected_cloud_list.add_vector_quantity("Gradient Vector (unprojected)", np.array(vector_list), color=(1.0,0,0), vectortype='ambient', enabled=True)
